...
void eLagoLearn( int *Y, double *Xr, double *Dr,  double *C0r, double *C1r, int *nr, int *pr, int *n0r, int *n1r, int *kr, double *ar, double *R )
{
...
//	if( ( tempR = ( pArr * ) Calloc( sizeof( Arr ) * n1, Arr ) ) == NULL )
//	if( ( tempR = ( pArr * ) malloc( sizeof( Arr ) * n1 ) ) == NULL )
	if( ( tempR = ( pArr * ) R_alloc( n1, sizeof( Arr )) ) == NULL )
	{
		printf("Error 104, Program Terminating...\n");
		exit(1);
	}

	for( i = 0; i < n1; i++ )
	{
//		if ( ( tempR[i] = ( pArr ) Calloc( sizeof( Arr ), Arr ) ) == NULL )
//		if ( ( tempR[i] = ( pArr ) malloc( sizeof( Arr ) ) ) == NULL )
		if ( ( tempR[i] = ( pArr ) R_alloc( 1,sizeof( Arr ) ) ) == NULL )
		{
			printf("Error 105.  Program Terminating...\n");
			exit(1);
		}
//		if( ( tempR[i]->arr = ( double * ) Calloc( sizeof( double ) * p, double ) ) == NULL )
//		if( ( tempR[i]->arr = ( double * ) malloc( sizeof( double ) * p ) ) == NULL )
		if( ( tempR[i]->arr = ( double * ) R_alloc( p,sizeof( double ) ) ) == NULL )
		{
			printf("Error 106.  Program Terminating...\n");
			exit(1);
		}
	}
...
//	if( ( index = ( int * ) malloc( sizeof( int ) * k ) ) == NULL )
	...
	for( i = 0; i < n1; i++ )	// for every observation in C1
	{
		...
		for( j = 0; j < p; j++ )
		{
			...
			{
				temp = index[h];
				sum = sum + fabs( C1[i]->arr[j] - C0[temp]->arr[j] );
			}
